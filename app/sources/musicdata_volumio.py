...
# musicdata service to read from Volumio V2&V3
...
class musicdata_volumiobase(musicdata.musicdata):
	def __init__(self, q, server=u'localhost', port=3000, exitapp = [ False ] ):
		super(musicdata_volumiobase, self).__init__(q)
...
#					self.musicdata[k] = u''
			...
			if int(self.musicdata[u'length']) > 0:
				timepos = time.strftime("%-M:%S", time.gmtime(int(self.musicdata['elapsed']))) + u"/" + time.strftime("%-M:%S", time.gmtime(int(self.musicdata['length'])))
...
class musicdata_volumio3(musicdata_volumiobase):
	sio = socketio.Client()

	def __init__(self, q, server=u'localhost', port=3000, exitapp = [ False ] ):
		self.init_callbacks()
		super(musicdata_volumio3, self).__init__(q)
...
#	pwd= ''
	...
	exitapp = [ False ]
	q = queue.Queue()
	mdr = musicdata_volumio3(q, server, port, exitapp)
	mdr.run()
